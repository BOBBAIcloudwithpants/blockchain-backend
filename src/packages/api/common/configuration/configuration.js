...
class Configuration {
  constructor(configFilePath) {
    ...
    this._parseContractDir(config);
    this._parseContractOutputDir(config);
  }
  _parseContractOutputDir (config) {
    if (!config.contractOutputDir || typeof config.contractOutputDir !== 'string') {
      throw new ConfigurationError('invalid `contractOutputDir` property');
    } else {
      let contrastDir = config.contractOutputDir;
      if (!path.isAbsolute(contrastDir) && this.configDir) {
        contrastDir = path.join(this.configDir, contrastDir)
      }
      this.contractOutputDir = contrastDir
    }
  }

  _parseContractDir (config) {
    if (!config.contractDir || typeof config.contractDir !== 'string') {
      throw new ConfigurationError('invalid `contractDir` property');
    } else {
      let contrastDir = config.contractDir;
      if (!path.isAbsolute(contrastDir) && this.configDir) {
        contrastDir = path.join(this.configDir, contrastDir)
      }
      this.contractDir = contrastDir
    }
  }
